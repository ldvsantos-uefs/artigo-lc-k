...
<!-- Lembrar de buscar referencia Estudos apontam que, o processo degradativo tem início na hemicelulose, fração amorfa e hidrofílica suscetível a ataques hidrolíticos [@Lv2023] -->. Neste sendito, a mercerização remove parcialmente esse componente, elevando a cristalinidade e a estabilidade química [@Yang2024]. Análises FTIR confirmaram a redução da banda em 1735 cm⁻¹ (hemiceluloses) nos grupos tratados, enquanto o índice de cristalinidade aumentou de 48,5% (controle) para 62,3% (T3). Esses dados corroboram que a modulação microestrutural amplia a resistência à degradação, alinhando-se a estudos em outras fibras lignocelulósicas [@Tanasa2022; @Acharya2024].

Quanto à deformação máxima ($\varepsilon_{\text{max}}$), nota-se que, enquanto as fibras naturais (T0) sofrem uma transição abrupta para um comportamento frágil após 60 dias, com a ductilidade caindo para níveis inferiores a 5% aos 180 dias, os tratamentos alcalinos (T2 e T3) sustentam deformabilidades superiores a 6%. Essa preservação parcial da ductilidade está associada à remoção de hemicelulose, cuja degradação foto-oxidativa em materiais não tratados tende a promover ligações cruzadas rígidas que fragilizam a matriz polimérica [@Manimaran2018].
...
Dados sobre a cinética de degradação da ductilidade reforçam a elevada vulnerabilidade das fibras naturais. A meia-vida de 19,5 dias, associada à queda para 10% da deformação inicial em apenas três dias, confirma que a hemicelulose (principal fase amorfa e higroscópica) atua como gatilho da fragilização, comportamento igualmente descrito para fibras de *Helicteres isora* e *Zea mays* submetidas a intemperismo [@Acharya2024; @Kavitha2023].
...
Literatura especializada corrobora essa transição de regime estocástico para determinístico, associando a remoção de hemicelulose e a reorganização cristalina à redução da variabilidade estocástica [@Acharya2024; @Zhang2020]. Valores de β superiores a 2,5 sinalizam a predominância de mecanismos de degradação controlados por difusão oxidativa, em detrimento de defeitos intrínsecos aleatórios [@Pickering2016].

Reflete-se no aumento simultâneo de β e η nos grupos tratados a maior previsibilidade mecânica e a recalcitrância química da matriz. A mercerização induz a conversão de Celulose I para Celulose II e aumenta o empacotamento molecular, reduzindo a difusão de espécies reativas e a energia superficial dos cristalitos [@Hiyama2006; @Yamane2006]. Essa reestruturação microestrutural, evidenciada pelo aumento do tamanho de cristalitos, torna as fibras menos vulneráveis ao ataque fotoquímico e à propagação de microfissuras, explicando a resistência ampliada observada em T2 e T3.
...
A validação estatística confirmou que a razão lignina/celulose (L/C) é o fator estruturante da cinética de degradação, explicando 82% da variância na taxa de degradação (β = -0.82; p < 0.001). O coeficiente padronizado demonstra que a vulnerabilidade intrínseca do material depende mais de sua composição molecular do que do tempo de exposição (β_std = +0,64). A análise de mediação indicou que 94,3% da influência da densidade de fraturas é transmitida pela razão L/C, evidenciando que as microfraturas são consequência da degradação química, e não sua causa primária.

Esse padrão corrobora a literatura, que identifica a oxidação da lignina como precursora da ruptura mecânica em compósitos naturais [@Kwon2021]. Os indicadores microestruturais (MEV) reforçam essa conclusão: o tratamento T3 reduziu significativamente a densidade de fraturas, compatível com a remoção de hemicelulose e o aumento da cristalinidade, que diminuem os pontos de nucleação de falhas [@Acharya2024; @Zhang2020]. Assim, a razão L/C atua como variável mestra, governando a recalcitrância química e, consequentemente, a integridade estrutural.
...
Os resultados permitem propor um protocolo de qualificação acelerada ancorado na recalcitrância química. A forte correlação entre índice de cristalinidade e o parâmetro $\beta$ de Weibull ($R^2 = 0,94$), combinada à associação entre marcadores de oxidação (FTIR) e cinética de fragilização ($r = 0,82$), sustenta o uso de ensaios laboratoriais rápidos (DRX, TGA, FTIR) para estimar a VUF, substituindo longas campanhas de envelhecimento.
...
Embora geossintéticos de polipropileno (PP) e poliéster (PET) apresentem energias de ativação superiores e maior estabilidade química [@Carneiro2018], sua persistência ambiental gera poluição por microplásticos [@Bai2022]. A *Typha* mercerizada ocupa um nicho para aplicações de médio prazo (90–180 dias), onde a biodegradabilidade programada é funcionalmente desejável [@Kwon2021], evitando o passivo ambiental de materiais sintéticos que persistem além de sua vida útil geotécnica.
...
O expoente $\gamma > 1$ reflete a aceleração fotoquímica decorrente da natureza autocatalítica da fotoxidação [@OrnaghiJr2019], onde produtos de degradação primária retroalimentam a cadeia oxidativa, amplificando a taxa de reação com o acúmulo de danos.
