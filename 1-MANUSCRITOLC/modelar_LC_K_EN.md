...
**Figure 3.** (a) Customized UV forced degradation chamber used for kinetic model calibration and (b) UV irradiation system with fluorescent lamps UV-A, UV-B, and visible.
...
**Figure 4.** Microstructural and mechanical characterization of degradation: (a) Temporal evolution of tensile strength and fracture patterns, (b) Representative stress-strain curves at 30 days and (c) at 90 days for all treatments.

![ ](../3-IMAGENS/figura5_paineis_ab.png){width="95%"}

### 3.2. Structural Reliability Analysis

Probabilistic durability analysis predicated upon Weibull distribution frameworks reveals that alkaline intervention substantially modifies fiber rupture physics, promoting transition from a regime dominated by stochastic failure to more deterministic behavioral patterns (Figure 5). Regarding tensile strength, estimated parameters indicate pronounced inter-group differentiation, wherein control T0 exhibited β = 2.3 and η = 68 days, whereas treatments T2 (6% NaOH) and T3 (9% NaOH) demonstrated β = 2.8; η = 94 days and β = 3.0; η = 92 days, respectively.

**Figure 5.** Cumulative reliability functions $R(t)$ derived from Maximum Likelihood estimation of Weibull parameters.
...
This mechanism agrees with that described by @Mansikkamäki2007, who demonstrated that the conversion of Cellulose I to Cellulose II is associated with increased crystallite size and greater chemical stability, making fibers less vulnerable to photochemical attack and microcrack propagation. Polymorphic transition represents a thermodynamically favorable reorganization of crystalline structure.

The molecular basis of this transition was elucidated through advanced spectroscopic techniques. @Hiyama2002 and @Atalla1984 confirmed through X-ray diffraction and solid-state nuclear magnetic resonance the rearrangement of intermolecular hydrogen bonds from parallel (Cellulose I) to antiparallel (Cellulose II) configuration, conferring greater entropic stability to the system.
...
**Figure 6.** Comparative scanning electron microscopy (SEM) analysis of natural (*Typha domingensis*) versus alkali-treated fibers (9% NaOH).

![ ](../3-IMAGENS/analise_mev_Typha_Domingensis_Natural.png){width="90%"}

*Note: Images at 500 µm × 500 µm scale show reduction in surface fractures, collapse of uncontrolled porosity, and crystalline reorganization in polymorphic pattern characteristic of successful mercerization.*
...
**Figure 8.** Decision flowchart for selection of *Typha domingensis* fiber treatment.
...
Substituting the values ($T_{\text{field}}=299.15\text{K}$, $T_{\text{chamber}}=313.15\text{K}$), an **Experimental Activation Energy ($E_a$) of 29.03 kJ/mol** was obtained. This value, lower than generic theoretical values for lignocellulose, reflects the actual susceptibility of *Typha domingensis* to combined hydrolysis and photo-oxidation, validating the need for protective treatments for long-term applications (Figure 9).

**Figure 9.** Activation Energy calibration via Arrhenius plot.

![ ](../3-IMAGENS/grafico_arrhenius_en.png){width="70%"}

#### 3.6.2. Accumulated Damage Model (Modified Paris-Erdoğan)

Damage evolution ($D$) was modeled by integrating the Weibull shape parameter ($\beta$) obtained experimentally in tensile tests ($\beta \approx 3.40$). The Paris-Erdoğan model was adapted to describe the damage growth rate as a function of strain energy density, analogous to fatigue approaches in fibrous composites [@Fazlali2021; @Moncy2022]:

$$
\frac{dD}{dN} = C (\Delta K)^m \rightarrow D(t) = 1 - \exp\left[ - \left( \frac{t}{\eta} \right)^\beta \right]
$$

Where the damage exponent $m$ was calibrated with the experimental $\beta$ parameter ($m \approx \beta = 3.40$). The hybrid simulation demonstrated that, for natural fiber, critical damage ($D_c = 0.5$) is reached in approximately 42 days in the field, consistent with observed data (FSL $\approx$ 42 days). For the treated composite, the simulation projects a service life extension exceeding 120 days, corroborated by the mechanical stability observed in chamber tests (no significant strength loss in 120 cycles), as depicted in Figure 10.

**Figure 10.** Accumulated damage evolution for natural and treated (6% NaOH) fibers according to the hybrid model.

![ ](../3-IMAGENS/grafico_dano_hibrido_en.png){width="80%"}

This cross-validation between field and laboratory confirms that the hybrid model adequately captures degradation physics, allowing safe extrapolations to different climatic scenarios.

#### 3.6.3. Comparative Analysis with Other Geosynthetics

The activation energy obtained for *Typha domingensis* ($E_a = 29.03$ kJ/mol) lies in an intermediate range when compared to other natural and synthetic fibers. Fibers with high lignin content, such as Coir, typically present higher recalcitrance, while fibers with lower lignin content, like jute, exhibit comparable or higher degradation rates [@Carvalho2014; @Thakur2019].

In contrast, polymeric geosynthetics such as polypropylene (PP) and polyester (PET) possess significantly higher activation energies, reflecting their superior chemical stability, albeit with environmental disadvantages associated with microplastic persistence [@Carneiro2018; @Franco2022]. Treated *Typha*, therefore, fills a specific niche for medium-term applications (3-6 months), where biodegradability is a functional requirement, differing from the multi-decade longevity of synthetics.

### 3.7. Limitations and Recommendations for Future Work

Mitigation of this limitation in high irradiance scenarios requires incorporation of a nonlinear correction term in the degradation constant (Equation 5).

$$
k_{\text{effective}} = k_0 \cdot (1 + \alpha \cdot UV^\gamma)
$$

Parameter $k_0$ represents basal degradation rate under laboratory conditions; $UV$ quantifies local normalized ultraviolet index; $\alpha$ captures material environmental sensitivity; $\gamma$ defines photochemical acceleration, typically greater than 1 for natural polymers due to the autocatalytic nature of photooxidation. Calibrating these additional parameters requires field tests at multiple climatic sites, constituting a priority for future works aiming to universalize the model for arid and semi-arid biomes, where UV irradiance systematically exceeds values observed in humid tropical environments.

**Figure 11.** Model validation via Monte Carlo simulation.
...
Mechanistic validation received support through convergence between model predictions and spectroscopic observations via FTIR, wherein carbonyl group (C=O) formation at 1735 cm⁻¹—diagnostic of hemicellulose oxidation—coupled with aromatic vibration reduction at 1505 cm⁻¹—indicative of lignin degradation—correlated positively ($r = 0.82$; $p < 0.001$) with ductility degradation. This result substantiates underlying chemical photooxidation as a plausible mechanism governing observed kinetics, wherein analogous findings documented by @Tanasa2022 for hemp fibers revealed that lignin photolysis functions as a trigger for secondary cell wall decohesion, thereby conferring physical legitimacy to the empirical model through robust spectroscopic-mechanical correlation.

Statistical power evaluation indicated that sample dimensionality positioned the experiment within an adequate sensitivity zone for detecting moderate to large magnitude effects (Cohen's $d = 0.6$), exhibiting statistical power of 80% ($\beta = 0.20$). For critical regulatory decisions and commercial product final validation, sample size augmentation to $n \ge 60$ is recommended, whereby such measure would elevate statistical power to values exceeding 90%, thereby minimizing Type II error risk (false negatives) in detecting premature failures. Explicit articulation of this trade-off provides readers with instruments to adjudicate whether presented conclusions possess sufficient rigor for their specific application contexts.

## Conclusions

Results obtained in this investigation enable the conclusion that the lignin/cellulose ratio (L/C) confirmed itself as the determinant factor governing degradation kinetics of *Typha domingensis* fibers, exerting influence superior to temporal exposure regarding mechanical property loss. Surface fracture density, analyzed via SEM, behaved as a secondary manifestation of this chemical degradation process.

Alkaline treatment employing 6% NaOH (T2) established itself as the operational equilibrium point, extending Functional Service Life (FSL) by 127% compared to control while preserving ductility necessary for geotechnical applications. Conversely, the 9% concentration, despite augmenting initial strength, induced excessive embrittlement, rendering the material less suitable for functions requiring deformation accommodation.

Microstructural and spectroscopic analysis corroborated the physicochemical stabilization mechanism, indicating that mercerization promoted polymorphic conversion from Cellulose I to II alongside crystallinity augmentation. These alterations correlated strongly with mechanical property retention, suggesting that chemical stability exerts significant influence upon macroscopic performance.

Additionally, morphological characterization revealed that alkaline treatment induces matrix reorganization which, although increasing apparent porosity, generates an ordered void structure that does not compromise mechanical strength. Simultaneously, physical property homogenization was observed, evidenced by tensile strength coefficient of variation reduction, which directly contributes to enhanced material reliability.

Reliability modeling via Weibull distribution indicated that chemical modification promotes a transition in failure behavior, reducing natural variability and enabling performance prediction predicated upon deterministic wear parameters.

Finally, hierarchical model validation demonstrated robustness across low and medium UV irradiance scenarios, enabling Functional Service Life (FSL) estimation from compositional parameters. However, for high radiation environments (UV index 1.0), increased predictive variability was observed, indicating the necessity for correction factors in applications involving arid climates or high altitude contexts.
